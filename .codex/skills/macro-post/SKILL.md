...
1.2 **Market-close financing balance requirement** — In market-close briefs, include a dedicated `融資餘額觀察` section whenever data is available. For Taiwan, report TWSE margin balance (融資餘額交易單位 + 融資餘額金額) with day-over-day direction. For US, report latest FINRA customer margin debt month and MoM direction. If unavailable, explicitly mark `資料暫缺` and do not fabricate.
